...
export default function AdminServicesPage() {
  ...
  return (
    <main>
      ...
      <p className="mt-1 text-sm text-muted-foreground">Edit catalog prices. New bookings pick up the change immediately.</p>
      ...
      <div className="mt-4 space-y-3">
        {services.map((service) => (
          <form
            ...
            className="grid gap-2 rounded-2xl border bg-card p-4 sm:grid-cols-[1fr_8rem_6rem_auto]"
          >
            <div>
              <p className="font-medium">{service.name}</p>
              <p className="text-xs text-muted-foreground">{service.category}</p>
            </div>
            <Input name="basePrice" type="number" step="0.01" min={1} defaultValue={service.basePrice} />
            <Input name="durationMin" type="number" min={10} defaultValue={service.durationMin} />
